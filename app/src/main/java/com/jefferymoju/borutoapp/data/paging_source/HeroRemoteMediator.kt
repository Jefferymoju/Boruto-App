...
class HeroRemoteMediator (
    ...
    override suspend fun initialize(): InitializeAction {
        val currentTime = System.currentTimeMillis()  // this will be compared to the lastUpdated value from our server whenever we make a new request
        val lastUpdated = heroRemoteKeysDao.getRemoteKeys(heroId = 1)?.lastUpdated ?: 0L // to get the lastUpdated from the server
        val cacheTimeout = 1440 // time to wait till refresh
        ...
        return try {
            ...
            if (response.heroes.isNotEmpty()){
                borutoDatabase.withTransaction {
                    ...
                    heroRemoteKeysDao.addAllRemoteKeys(heroRemoteKeys = keys) // with this fun we are adding all our heroes with its keys to our hero remote keys database table
